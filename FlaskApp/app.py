...
def getRecommendations():
	...
	sql = 'SELECT movieID, title, year from Movies m, (select movie_id, count(movie_id) as movie_count from UserLikes u, (select Following from Following f where f.Follower="{}") f where f.Following = u.user_id group by movie_id order by movie_count DESC LIMIT 10) A where A.movie_id = m.movieID'.format(user.id)
	# Execute SQL
	cursor.execute(sql)
	# Collect Results
	tuples = cursor.fetchall()
